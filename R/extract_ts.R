...
#' 
#' # freq contour 95 dif lengths
#' extract_ts(X = selection_file_ts, ts.column = "Freq.Contour.95...Hz.")
#' 
#' # freq contour 95 equal lengths
#' extract_ts(X = selection_file_ts, ts.column = "Freq.Contour.95...Hz.", equal.length = T)
#' 
#' # freq contour 95 equal lengths
#' extract_ts(X = selection_file_ts, ts.column = "Peak.Freq.Contour..Hz.", equal.length = T)   
#' }
#' 
...
extract_ts <- function(X, ts.column, equal.length = FALSE, as.time.series = FALSE){
  ...
  if(equal.length)
  {
    out <- lapply(out, function(x) stats::approx(x, 1:length(x), n = max(sapply(out, length)))$x)
  ...
  if(as.time.series) {
    ...
    return(Y)} else {
      ...
      Z <- Z[, c(grep("file", names(Z), ignore.case = TRUE), grep("file", names(Z), invert = TRUE, ignore.case = TRUE), 3:ncol(Z))]
